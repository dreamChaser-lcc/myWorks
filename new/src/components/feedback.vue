...
<div class="container">
      ...
      <el-table :data="data_FB" border style="box-shadow:10px 10px 5px #e3e3e3;" ref="filterTable" 
       ...
       >
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
              <el-button icon="el-icon-chat-dot-round" @click="detailFB(scope.row)"></el-button>
              <el-button icon="el-icon-s-promotion" @click="editFB(scope.row)">处理</el-button>
              <el-button icon="el-icon-delete" @click="deleteFB(scope.row)"></el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="反馈信息详情" :visible.sync="handleDtailDialog" width="30%" >
            <div style="font-size:15px">
             <strong style="display:inline-block;width:80px;text-align:center">
               用户名：</strong><span>{{this.detailFbData.username}}</span>
            <el-divider></el-divider>
            <strong style="display:inline-block;width:80px;text-align:center">
               反馈类型：</strong><span>{{this.detailFbData.fbtype}}</span>
            <el-divider></el-divider>
            <strong style="display:inline-block;width:80px;text-align:center">
               反馈内容：</strong><span>{{this.detailFbData.content}}</span>
            <el-divider></el-divider>
             <strong style="display:inline-block;width:80px;text-align:center">
               联系方式：</strong><span>{{this.detailFbData.contacts}}</span>
            <el-divider></el-divider>
            <strong style="display:inline-block;width:80px;text-align:center">
               处理状态：</strong><span>{{this.detailFbData.state?"已处理":'未处理'}}</span>
            <el-divider></el-divider>
            <strong style="display:inline-block;width:80px;text-align:center">
               {{this.detailFbData.state?"处理":'反馈'}}时间：</strong><span>{{this.detailFbData.time}}</span>
            <el-divider></el-divider>
            </div>
    </el-dialog>
     <!-- 页码部分 -->
...
watch:{
  ...
  methods: {
    ...
    async getDisHandle(){
       let {data:num} = await this.$http.get("/getDisHandle"); 
       if(num>0){
           this.$notify({
                title: '提示',
                message: `当前有${num}条反馈信息未处理,请查看`,
                duration: 0
              });
       }
    },
